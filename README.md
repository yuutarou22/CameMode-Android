...
アプリ名：CameMode
1. **「モデルさん募集してます」と言う前、「モデ」と発言した時点で見つかるほど早く目的を叶えられる** という願いを込めて（その逆であるカメラマンも同様）。
2. 「Came（来た）」の意味を含む。
...
……ということで、同じ意思を持った（撮りたい、撮られたい）人を集めたいという思いから制作に着手した。

## アプリの工夫点

学習目的の制作でも、「使ってもらえる」アプリを制作するため以下の差別化を図った。

1. Twitterでの募集、声かけだと非効率な部分を、このアプリで絞り込み化！
- Twitter上にはハッシュタグが存在するが、数が多すぎる。
  - 飽和状態のため、目につく機会が少ない。すぐに流れてしまう。
  - 複数のハッシュタグをつけるという対策もあるが、煩わしい。
    - このアプリで絞り込み化を実現。カメラマン、モデルは集まれ！

2. Twitterだとフォロワー数、拡散数などに左右されやすいため、人目につかない→このアプリで絞り込み化
- 上記の1と同様、絞り込み化を行う。
  - カメラマン、モデル募集目的に特化したアプリにすることで、同じ目的の人を集める。

3. アカウント登録が簡易的
  - アプリ内でのコミュニケーション機能は**無い**。
  - そのため、ユーザそれぞれが利用するSNSでの **「繋がるきっかけ作り」** を目的としている。
    - したがって当アプリ専用アカウントの登録は、基本的な情報の登録のみを行う。
  ※SNS認証を実装するか検討中
...
- スプラッシュ画面の実装
  - 横長のロゴとアイコンの画像を表示させようとしたが、うまくいかなかった。
    - ブランチ：0032_スプラッシュの実装（CameModeリポジトリ）
  - 以下のサイトを参考に実装している
    - https://minpro.net/splash-screen
    - http://sakebook.hatenablog.com/entry/2016/06/01/031950
    - http://chiiia12.hatenablog.jp/entry/2018/10/30/184328
    - https://developer.android.com/guide/topics/resources/drawable-resource?hl=ja
    - https://dev.classmethod.jp/smartphone/android/android-ui-design-layer-list/
